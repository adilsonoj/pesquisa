...
//$grafico->SetTitle("Média do grau por questão");
//$grafico->SetXTitle("QUESTÃO");
//$grafico->SetYTitle("MÉDIA");

//$grafico->SetLegend("Média");

//$grafico->SetXLabel("MÉDIA");
...
$sql_pergunta = oci_parse($ora_conn,"SELECT * FROM pergunta ORDER BY numero");
			...
			while ($row_pergunta = oci_fetch_assoc($sql_pergunta)) {
				...
				if (!$row_media['MEDIA']==NULL){
					array_push($array, array($id_pergunta, $row_media['MEDIA']));
				}

			}


$grafico->SetDataValues($array);
